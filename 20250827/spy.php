...
/**
 * SPY 近 30 年：每年最後一個交易日 Close / Adj Close、年報酬率、30 年 CAGR
...
$ticker  = 'SPY';
$period1 = strtotime('1993-01-01');
$period2 = time();
$yahooHistoryPage = 'https://finance.yahoo.com/quote/SPY/history?p=SPY';
...
$stooqUrl = 'https://stooq.com/q/d/l/?s=spy.us&i=d'; // 備援：日線 CSV (無 Adj Close)
...
if ($useStooq) {
    ...
    // 去掉 UTF-8 BOM（有些伺服器會帶）
    $sData = preg_replace('/^\xEF\xBB\xBF/', '', $sData);

    // 解析 Stooq CSV：常見標頭為 Date,Open,High,Low,Close,Volume（首字母大寫）
    ...
    if (!isset($idx['date']) || !isset($idx['close'])) {
        // 有些 mirror 可能用分號或奇怪分隔，改用更寬鬆方式再讀一次
        // 嘗試以 CRLF 分行、用 explode ',' 解析
        $lines = preg_split("/\r\n|\n|\r/", $sData);
        if (count($lines) > 1) {
            $first = trim($lines[0]);
            $firstLower = strtolower(str_replace(' ', '', $first));
            if (strpos($firstLower, 'date') !== false && strpos($firstLower, 'close') !== false) {
                // 仍算通過；但以 fgetcsv 的結果為準
            } else {
...
echo "SPY 近 30 年：每年最後交易日收盤價與報酬率（Close / Adj Close）\n";
echo str_pad("Year", 6) .
    str_pad("Date", 14) .
    str_pad("Close", 12) .
    str_pad("AdjClose", 12) .
...
// JSON 輸出
$jsonPath = __DIR__ . '/spy_last_trading_day_30y.json';
$output = [
    'data' => $result,
    'cagr' => [
...
// CSV 輸出
$csvPath = __DIR__ . '/spy_last_trading_day_30y.csv';
$fp = fopen($csvPath, 'w');
